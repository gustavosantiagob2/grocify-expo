...
type ItemsReponse = { items: GroceryItem[] };
...
export const useGroceryStore = create<GroceryStore>((set, get) => ({
  ...
  loadItems: async () => {
    ...
    try {
      ...
      const payload = (await res.json()) as ItemsReponse;
  ...
  addItem: async (input) => {
    ...
    try {
      ...
      if (!res.json) throw new Error(`Request failed (${res.status})`);

      set((state) => ({ items: [payload.item, ...state.items] }));
      return payload.item;
    } catch (error) {
      console.error("Erro adding item:", error);
      set({ error: "Something went wrong" });
    }
  },
  ...
  removeItem: async (id) => {
    ...
    try {
      ...
      console.log("Erro romoving item", error);
      set( { error: "Something" });
    }
  },
